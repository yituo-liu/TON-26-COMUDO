...
Kappa = torch.tensor(1e-4, device=device)
...
for t in range(T-1):
        ...
        with torch.no_grad():
            for param_tensor, param in local_models[i].named_parameters():
                if param.requires_grad and param.grad is not None:
                    param.data.copy_(models_agg[i][param_tensor] - (param.grad + 2 * Kappa * param.data) * 0.5)
    ...
    for i in range(N):
        state_dict = local_models[i].state_dict()
        neighbors = torch.where((current_weights_matrix[:,i] > 0) & (torch.arange(N, device=device) != i))[0]
        Xi[i] = torch.sum(Lambda[i,:]) / torch.maximum(torch.count_nonzero(Lambda[i,:]), torch.tensor(1, device=device))
        transmit_power += E[i] ** 2 * ratio[i]

    tranPower_toHistory = (transmit_power_history[t] * (t+1) + transmit_power.item()/(N*48910/500))/(t+2)
