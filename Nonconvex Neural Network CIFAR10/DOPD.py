...
Lambda = torch.tensor(2.0e-5, device=device)
Gamma = torch.tensor(2e-2, device=device)
...
g_gradient = {
        'layer3.0.conv1.weight': None,
        'layer3.0.conv2.weight': None,
        'layer3.1.conv1.weight': None,
        'fc.weight': None,
        'fc.bias': None,
    }
...
for i in range(N):
    ...
    previous_weights = weight_matrix[0]
    ...
    for param_tensor, param in local_models[i].named_parameters():
        ...
        for j in range(N):
            if j in neighbors:
                ...
                gn = gn + power_compute
    Gtn[i] = torch.maximum(gn - Pnbar, torch.tensor(0, device=device))
    Gtn[i] = torch.tensor(1e-2, device=device)
...
for t in range(T-1):
    ...
    for i in range(N):
        ...
        with torch.no_grad():
            for param_tensor, param in local_models[i].named_parameters():
                if param.requires_grad and param.grad is not None:
                    param.data.copy_(models_agg[i][param_tensor] - param.grad * 0.05)
        ...
        if Gtn[i] > 0:
            ...
            with torch.no_grad():
                for param_tensor, param in local_models[i].named_parameters():
                    if param.grad is not None:
                        param.data.copy_((models_agg[i][param_tensor] - 0.05 * param.grad) /(1 + 0.05 * Gamma * Gtn[i] * g_gradient[param_tensor]))
    ...
    tranPower_toHistory = (transmit_power_history[t] * (t+1) + transmit_power/(N*92810/500))/(t+2)
