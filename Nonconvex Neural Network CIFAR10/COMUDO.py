...
Gamma = torch.tensor(4.5e-3, device=device)
Eta = torch.tensor(1e-3, device=device)
V = torch.tensor(2, device=device)
Qtn = {i: torch.tensor(V, device=device) for i in range(N)}
Gtn = {i: torch.tensor(0.0, device=device) for i in range(N)}
    ...
    for i in range(N):
            ...
            for param_tensor, param in local_models[i].named_parameters():
                for j in neighbors:
                    if param.grad is not None:
                        ...
                        g = torch.sum((Lambda * (1/N) * param.data * h_abs_inverse) ** 2)
                        g_sum += g
        Gtn[i] = g_sum - Pnbar
        ...
        if Gtn[i] > 0:
            ...
            with torch.no_grad():
                for param_tensor, param in local_models[i].named_parameters():
                    if param.grad is not None:
                        param.data.copy_((models_agg[i][param_tensor] - 0.05 * param.grad) /(1 + 0.05 * Gamma * Qtn[i] * g_gradient[param_tensor]))
