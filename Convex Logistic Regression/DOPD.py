...
def DOPD(N, total_step, D, d, C, Data, Label, H, Z):
    slot = 7840 / 500
    ...
    for step in range(1, total_step):
        ...
        for n in range(N):
            ...
            if g_Power_test > 0:
                ...
                GD_g = 2 * (lamda**2) * temp 
                Q = gamma * g_Power_test
                xtn[step][n] =(ytn[step - 1][n] - 1 / (2 * alpha) * GD_tnc_f[n]) / (1 + 1 / (2 * alpha) * Q * GD_g)
...
Acc_DOPD, Power_Vio_DOPD = DOPD(N, total_step, D, d, C, Data, Label, H, Z)
