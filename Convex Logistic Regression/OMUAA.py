...
def OMUAA(N, total_step, D, d, C, Data, Label, H, Z):
    slot = 7840 / 500
    lamda = 2e-4
    ...
    Qtn = {t: np.zeros((N,))  for t in range(0, total_step)}
    ...
    for step in range(1, total_step):
        ...
        for n in range(N):
            ...
            gtn[step][n] = Power_n - Pn_bar
            Qtn[step][n] = max(Qtn[step - 1][n] + gtn[step][n], 0)
...
Acc_OMUAA, Power_Vio_OMUAA = OMUAA(N, total_step, D, d, C, Data, Label, H, Z)
